...
export default function SuggestSourcePage() {
  ...
  return (
    ...
    >
      <AppSection>
        <div className="mb-5 flex items-start gap-3 rounded-xl bg-emerald/5 px-4 py-3 text-[13px] text-muted">
          ...
          <p>
            ...
            <strong className="text-ink">Telegram-канали</strong>. Якщо ви знаєте корисний канал або
            майданчик — надішліть посилання. Підтримка:{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="source-url" className="mb-1.5 block text-[12px] font-semibold text-ink">
              Посилання *
            </label>
            <input
              id="source-url"
          ...
          <div>
            <label htmlFor="source-comment" className="mb-1.5 block text-[12px] font-semibold text-ink">
              Коментар
            </label>
            <textarea
              id="source-comment"
          ...
          <Button type="submit" variant="emerald" size="md" disabled={submitting}>
      ...
      <AppSection className="mt-6">
        <h2 className="text-[15px] font-bold text-ink">Ваші заявки</h2>
        <p className="mt-1 text-[12px] text-muted">Історія пропозицій і їхній статус.</p>

        {loading ? (
          <p className="mt-6 text-center text-[13px] text-muted">Завантаження...</p>
        ) : items.length === 0 ? (
          <AppEmpty className="mt-6">Ще немає заявок — надішліть першу пропозицію вище.</AppEmpty>
        ) : (
          <ul className="mt-5 space-y-3">
            {items.map(item => (
              <li
                key={item.id}
                className="rounded-xl border border-border/60 bg-white px-4 py-3.5"
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <a
                    ...
                    className="text-[13px] font-semibold text-emerald-dark hover:underline break-all"
                  >
                    {item.url}
                  </a>
