...
# Скільки AUTO.RIA ID тягнути в live-пул (/auto/search — лише IDs, без /auto/info).
# Гідратація — лише поточна сторінка в slice_pool.
AUTO_RIA_ID_COLLECT_CAP = 2500
# Live pool збирає лише IDs — для 2500 ID потрібно більше часу на пагінацію search API.
AUTO_RIA_POOL_TIMEOUT_SECONDS = 25.0
IMPERIYA_POOL_TIMEOUT_SECONDS = 12.0
...
UDRIVE_POOL_TIMEOUT_SECONDS = 12.0
UDRIVE_PAGE_SIZE = 50
UDRIVE_POOL_MAX_PAGES = 3
CAR_MARKET_POOL_TIMEOUT_SECONDS = 12.0
CAR_MARKET_PAGE_SIZE = 20
LUBEAVTO_POOL_TIMEOUT_SECONDS = 12.0
LUBEAVTO_PAGE_SIZE = 24
REONO_POOL_TIMEOUT_SECONDS = 12.0
AUTO_RIA_BETA_POOL_TIMEOUT_SECONDS = 25.0
REONO_PAGE_SIZE = 40
# TG — лише БД (без keyword refresh на live-пошуку).
TELEGRAM_POOL_TIMEOUT_SECONDS = 12.0
# Перший live-пошук: одна-дві сторінки з кожного майданчика, не весь каталог.
LIVE_SOURCE_FIRST_CAP = 48
...
def _source_token(raw: str) -> str:
    return raw.strip().lower().replace(".", "_").replace(" ", "_").replace("-", "_")


_LUBEAVTO_SOURCE_KEYS = frozenset(
    {
        "lubeavto",
        "lube_avto",
        "любе_авто",
...
def normalize_sources(sources: list[str] | None) -> list[str]:
    default = ["auto_ria", "olx", "car_market", "lubeavto", "reono", "imperiya", "udrive"]
    ...
    for raw in sources:
        ...
        elif key in ("imperiya", "imperiya_auto", "iautos"):
            normalized.append("imperiya")
        elif key in ("udrive", "u_drive"):
            normalized.append("udrive")
        elif key in ("car_market", "carmarket", "car_market_net"):
            normalized.append("car_market")
        elif key in _LUBEAVTO_SOURCE_KEYS:
            normalized.append("lubeavto")
        elif key in ("reono", "reono_ua"):
...
def sources_for_filters(filters: SearchFilters) -> list[str]:
    ...
    if category == "new":
        without_slow = [source for source in sources if source not in ("olx", "car_market", "lubeavto", "reono")]
        if without_slow:
            return without_slow
    return sources
...
async def _search_single_source(
    ...
    *,
    ...
    if source == "car_market":
        return await search_car_market(
            filters,
            page=page,
            per_page=per_page,
            sort_by=sort_by,
    ...
    if source == "auto_ria_beta":
        return await search_auto_ria_beta(
            filters,
            page=page,
            per_page=per_page,
            sort_by=sort_by,
...
async def _fetch_source_pool(
    ...
    *,
    ...
    if source == "car_market":
        return await fetch_car_market_pool(
            filters,
            need=need,
            sort_by=sort_by,
            use_cache=use_cache,
    ...
    if source == "auto_ria_beta":
        return await fetch_auto_ria_beta_pool(
            filters,
            need=min(need, AUTO_RIA_BETA_PAGE_SIZE),
            sort_by=sort_by,
            use_cache=use_cache,
            cache_ttl_seconds=cache_ttl_seconds,
            html_pages=1,
    ...
    if source == "lubeavto":
        return await fetch_lubeavto_pool(
            filters,
            need=need,
            sort_by=sort_by,
            use_cache=use_cache,
...
async def search_listings_outcome(
    ...
    *,
    ...
    async def run_car_market() -> PaginatedListings | Exception:
        try:
            return await asyncio.wait_for(
                _fetch_source_pool(
                    "car_market",
                ...
                timeout=CAR_MARKET_POOL_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
            return TimeoutError(f"Car Market: таймаут {CAR_MARKET_POOL_TIMEOUT_SECONDS:.0f}s")
        except Exception as exc:
            return exc
    ...
    async def run_lubeavto() -> PaginatedListings | Exception:
        try:
            return await asyncio.wait_for(
                ...
                timeout=LUBEAVTO_POOL_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
            return TimeoutError(f"Любе Авто: таймаут {LUBEAVTO_POOL_TIMEOUT_SECONDS:.0f}s")
        except Exception as exc:
            return exc
    ...
    if "lubeavto" in sources:
        ...
        if isinstance(lubeavto_out, BaseException):
            ...
            successful.append(("lubeavto", lubeavto_out))
            source_statuses.append(
                SourceSearchStatus(source="Любе Авто", item_count=len(lubeavto_out.items))
            )
...
def _listing_to_slot(item: ListingOut) -> dict:
    ...
    if src == "lubeavto" or lid.startswith("lubeavto_"):
        return {"s": "l", "d": item.model_dump(mode="json")}
...
def _ar_listings_from_html_cards(
    auto_ria_ids: list[str],
    html_cards: dict[str, ListingOut] | None,
) -> list[ListingOut]:
    cards = html_cards or {}
    items: list[ListingOut] = []
    for aid in auto_ria_ids:
        raw = aid[2:] if aid.startswith("n:") else aid
        card = cards.get(raw) or cards.get(aid)
        if card is not None:
            items.append(card)
    return items
...
def _build_fair_blend_slots(
    *,
    ...
    auto_ria_beta_items: list[ListingOut] | None = None,
    lubeavto_items: list[ListingOut] | None = None,
    reono_items: list[ListingOut] | None = None,
    limit: int,
    sort_by: str = "newest",
) -> list[dict]:
    """Round-robin OLX → Car Market → Любе Авто → REONO → Імперія → uDrive → Telegram → AUTO.RIA; усередині кожного — sort_by."""
    imperiya_items = imperiya_items or []
    udrive_items = udrive_items or []
    car_market_items = car_market_items or []
    auto_ria_beta_items = auto_ria_beta_items or []
    lubeavto_items = lubeavto_items or []
    ...
    if auto_ria_beta_items:
        batches.append(
            (
                "auto_ria_beta",
                [_listing_to_slot(item) for item in sort_listings(auto_ria_beta_items, sort_by)],
            )
        )
    if lubeavto_items:
        batches.append(
            (
                "lubeavto",
                [_listing_to_slot(item) for item in sort_listings(lubeavto_items, sort_by)],
            )
        )
    if reono_items:
        batches.append(
...
def _build_interleaved_slots(
    *,
    ...
    return _build_fair_blend_slots(
        ...
        auto_ria_beta_items=auto_ria_beta_items,
        lubeavto_items=lubeavto_items,
...
async def build_live_search_pool(
    ...
    *,
    ...
    source_need = min(max(max_ids, 1), LIVE_SOURCE_FIRST_CAP)
    ...
    async def run_car_market():
        try:
            return await asyncio.wait_for(
                fetch_car_market_pool(
                    filters,
                    need=source_need,
                    sort_by=sort_by,
                ),
    ...
    async def run_lubeavto():
        try:
            ...
            logger.warning("Любе Авто pool fetch failed: %s", exc)
            return _empty_page(1, max_ids)
    ...
    if "lubeavto" in task_order:
        ...
        if isinstance(res, BaseException):
            lubeavto_result = _empty_page(1, max_ids)
            source_statuses.append(_failed_source_status("Любе Авто", res))
        else:
            lubeavto_result = res
            source_statuses.append(
                SourceSearchStatus(source="Любе Авто", item_count=len(lubeavto_result.items))
            )
    ...
    if brand_model_filter and (filters.model or "").strip() and not html_cards:
        try:
    ...
    if auto_ria_ids and (
        ...
        ar_html_items = _ar_listings_from_html_cards(auto_ria_ids, html_cards)
        if ar_html_items:
            merged = mark_duplicates_in_pool([*ot_merged, *ar_html_items])
            slots = [
            ...
            for slot in slots:
                if slot.get("s") == "n" and slot.get("i"):
                    represented.add(f"n:{slot['i']}")
                elif slot.get("s") == "r" and slot.get("i"):
                    represented.add(str(slot["i"]))
            for aid in auto_ria_ids:
                if len(slots) >= POOL_LIMIT:
                    break
                if aid in represented:
                    continue
                slots.extend(_make_ar_slots([aid], html_cards)[:1])
        else:
    ...
    return slots, nav_total, market_total, source_statuses, beta_cursor
