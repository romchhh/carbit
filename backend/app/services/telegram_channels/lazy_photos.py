...
async def attach_photos_to_listing(
    ...
    *,
    ...
    limit = max_photos if max_photos is not None else settings.TELEGRAM_MAX_PHOTOS
    try:
        paths = await service.download_listing_photos(listing_id, channel, message_ids)
    except Exception:
